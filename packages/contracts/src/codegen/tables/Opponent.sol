...
library Opponent {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "app", name: "Opponent", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x746261707000000000000000000000004f70706f6e656e740000000000000000);
   ...
   */
  function get(bytes32 id) internal view returns (OpponentData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = id;
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 id) internal view returns (OpponentData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = id;
   ...
   */
  function set(bytes32 id, OpponentData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 id, OpponentData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.y);
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (OpponentData memory _table) {
